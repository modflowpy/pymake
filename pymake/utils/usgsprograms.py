...
class usgs_program_data:
    ...
    def export_json(
        ...
        pl.Path(fpth).parent.mkdir(parents=True, exist_ok=True)
        ...
        pl.Path(appdir).parent.mkdir(parents=True, exist_ok=True)
        ...
        if prog_data is not None and write_markdown:
            with open(pl.Path(fpth).parent / "code.md", "w") as file_obj:
